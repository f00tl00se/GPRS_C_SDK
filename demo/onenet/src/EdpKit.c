...
//  * 
//  */
// typedef struct memHook {
// 	void *(*malloc_fn)(size_t sz);
// 	void (*free_fn)(void *ptr);
// } EDP_MEM_HOOK;

// /**
//  * @brief 动态内存申请、释放函数指针默认值
//  * 
//  */
// static void *(*EDP_MallocBuffer)(size_t sz) = malloc;
// static void (*EDP_FreeBuffer)(void *ptr) = free;

// /**
//  * @brief 初始化动态内存申请、释放函数指针
//  * 
//  * @param hook 动态内存函数钩子指针
//  */
// void EDP_MemHookInit(EDP_MEM_HOOK *hook) {
// 	if (!hooks) { /* Reset hooks */
//         EDP_MallocBuffer	= malloc;
//         EDP_FreeBuffer		= free;
//         return;
//     }

// 	EDP_MallocBuffer	= (hooks->malloc_fn)?hooks->malloc_fn:malloc;
// 	EDP_FreeBuffer	 	= (hooks->free_fn)?hooks->free_fn:free;
// }
uint32_t mystrlen(const int8* str)
{
    if (str)
    {
		return strlen(str);
...
//	说明：		
//==========================================================
uint8 EDP_PacketConnect1(const int8 *devid, const int8 *apikey, uint16 cTime, EDP_PACKET_STRUCTURE *edpPacket)
{
	
	uint8 devid_len = mystrlen(devid);
	uint8 apikey_len = mystrlen(apikey);
...
//	说明：		
//==========================================================
uint8 EDP_PacketConnect2(const int8 *proid, const int8 *auth_key, uint16 cTime, EDP_PACKET_STRUCTURE *edpPacket)
{
	
	uint8 proid_len = mystrlen(proid);
	uint8 authkey_len = mystrlen(auth_key);
...
uint8 EDP_PacketSaveData(const int8 *devid, int16 send_len, int8 *type_bin_head, SaveDataType type, EDP_PACKET_STRUCTURE *edpPacket)
{
	...
	if(type == 2)
		EDP_NewBuffer(edpPacket, mystrlen(type_bin_head));
	else
	...
	{
		if(type == 2)
			remain_len = 12 + mystrlen(type_bin_head) + send_len;
		else
	...
	{
		if(type == 2)
			remain_len = 10 + mystrlen(type_bin_head) + send_len;
		else
	...
	if(type == 2)
	{
		uint8 type_bin_head_len = mystrlen(type_bin_head);
		uint8 i = 0;
...
uint8 EDP_PacketPushData(const int8 *devid, const int8 *msg, uint32 msg_len, EDP_PACKET_STRUCTURE *edpPacket)
{
	...
	uint32 remain_len = 2 + mystrlen(devid) + msg_len;
	uint8 devid_len = mystrlen(devid);
	uint16 i = 0;
	uint16 size = 5 + mystrlen(devid) + msg_len;
...
//	说明：		
//==========================================================
uint8 EDP_PacketCmdResp(const int8 *cmdid, uint16 cmdid_len, const int8 *resp, uint32 resp_len, EDP_PACKET_STRUCTURE *edpPacket)
{
...
//	说明：		
//==========================================================
uint8 EDP_PacketPing(EDP_PACKET_STRUCTURE *edpPacket)
{
